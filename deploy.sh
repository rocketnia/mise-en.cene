...
git remote add rocketnia-upstream \
  "https://$ROCKETNIA_GH_TOKEN@github.com/rocketnia/mise-en.cene.git"
git fetch -q rocketnia-upstream
git reset rocketnia-upstream/gh-pages || true

# We build the dependencies of the demo. We replace the `.gitignore`
# file with a version where these dependencies are not ignored.
echo Building dependencies of the demo...
npm run build
cp gh-pages.gitignore .gitignore
echo Finished building dependencies of the demo.
